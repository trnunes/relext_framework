...
module Repositories
    ...
    class SingleTableRepository
        ...
        def get_connection
            ...
            @conn ||= mysql_driver.connect(@connection_string, nil)
            @conn
        end
        ...
        def find_dependency_list(sentence)
            ...
            def find_features(sentence_id)
                query = "select value, type from features where stc_id = #{sentence_id}"
                rs = execute_query(query)
                features = []
                while rs.next do
                    feature_value = rs.getString(1)
                    feature_type = rs.getString(2)
                ...
                def save_dependency_list(dependency_list, sentence)
                    dependency_list.each{|typed_dependency|
                        ...
                        execute_update(query, [stc_id, rel, gov, dep])
                    }
                end
                ...
                def find_sentence_by_relation(relation, offset, limit)
                    sql = "select * from examples s where s.relation = '#{relation}' LIMIT #{limit} OFFSET #{offset}"
                    
                    mount_sentence_list(execute_query(sql))
                end
                
                def find_sentences(offset, limit)
                    sql = "select * from examples s LIMIT #{limit} OFFSET #{offset}"
                    mount_sentence_list(execute_query(sql))
                end
            ...
            class MysqlRepository
                ...
                def find_all_gov_nodes
                    ...
                    def find_sentence_by_relation(relation, limit = nil, offset = nil)
                        sql = "select * from sentences s where s.property like '%#{relation}'" + " and s.id in (select stc_id from sentence_paths)"
